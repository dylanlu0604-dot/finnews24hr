...
長度限制：在每個部分中，「只能有一段文字」，請保持簡潔扼要，只關注最重要的發展。
個股限制：
- 在每個部分中，不需要提及個別股票，除非它們對整體市場情緒有重大影響。
- 忽略任何廣告訊息。
- 只關注每日發生的事情，不需要對未來進行預測。
- 每個部分保持簡潔扼要，只關注最重要的發展。
- 標題請創建一個簡潔的新聞式標題，清楚總結發生了什麼，不提及具體日期。
特殊條件：除非澳洲金融市場有特別大的波動或央行利率行動，否則不需要特別寫出澳洲市場。
        如果總體經濟數據包含明確的數字，請清楚列出這些數值。
...
- 只選擇多篇報告「都有」提到的經濟數據、資產進行摘要即可，如果只有一篇報告提到，不需要列入進行摘要。
